...
Так же, нужно добавить Webhooks. Можно не создавать отдельный продукт, а добавить его в самой вкладке Messenger (в таком случае выбор настроек не окажется таким обширным, но этого будет достаточно в рамках поставленной задачи).
...
Сообщения принимаются с помощью объекта **event**. Этот объект состоит из таких полей:

**sender**: *id* отправителя, 

**recipient**: *id* получателя,

**timestamp**: время отправления сообщения (необходимо переводить в UTC формат), 

**message**: *mid* - идентефикатор сообщения; *seq* - последоватеьность сообщения; *text* - текст сообщения или *attachments* с полями *type* - тип вложения, *payload* с подполем *url* - адресом вложения.

*Прим.: text заменяется на attachments в случае, если в чат было отправлено вложение, а не текст.*

Записываем в логи то, что приходит от пользователей (можно вывести и весь event целиком). Сохраняем файл index.js и разворачиваем нашу базу данных с помощью firebase deploy.
...
После этого создался Webhook, выберем страницу, с которой он будет отсылаться (Testpublicchat), подпишемся на нее, попробуем что-нибудь написать в этот чат от своего имени. Смотрим в журналы функций и видим:
